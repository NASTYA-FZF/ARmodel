...
void Drawer::DrawTwoSig(vector<double>& data, vector<double>& data1, signal s, CString Ox, CString Oy, double data_x_max, double step_x)
{
	...
	CPen two_pen(PS_SOLID, 2, RGB(0, 255, 20));
	...
	memDC.MoveTo(convert_range_d(s.GetL() / 2 * step_x, actual_right, actual_left, data_x_max, data_x_min), y[0]);

	for (unsigned int i = 1; i < y.size(); i++)
	{
		memDC.LineTo(convert_range_d((double)i * step_x + s.GetL() / 2 * step_x, actual_right, actual_left, data_x_max, data_x_min), y[i]);
	}

	memDC.SelectObject(&three_pen);

	memDC.MoveTo(actual_left, convert_range_d(s.GetE0(), actual_top, actual_bottom, data_y_max, data_y_min));

	memDC.LineTo(actual_right, convert_range_d(s.GetE0(), actual_top, actual_bottom, data_y_max, data_y_min));
